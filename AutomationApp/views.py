...
def marketingaspect(request):
    ...
    try:
        ...
        while juncountRO<juneROii.count():
            juneROi=Sales.objects.filter(DateTime__year='2022',DateTime__month='06' or '05',CusName=juneROii[juncountRO]).annotate(date=TruncDate('DateTime'),day=TruncDay('DateTime'),hour=TruncHour('DateTime'),minute=TruncMinute('DateTime')).values_list('date', flat=True).distinct().count()
            if juneROi==2:
                MAINjunROcounts+=1
            juncountRO+=1
    except Sales.DoesNotExist:
    ...
    try:
        ...
        while novcountRO<novemberROii.count():
            novemberROi=Sales.objects.filter(DateTime__year='2022',DateTime__month='11' or '10',CusName=novemberROii[novcountRO]).annotate(date=TruncDate('DateTime'),day=TruncDay('DateTime'),hour=TruncHour('DateTime'),minute=TruncMinute('DateTime')).values_list('date', flat=True).distinct().count()
            if novemberROi==2:
                MAINnovROcounts+=1
            novcountRO+=1
    ...
    try:
        ...
        while deccountRO<decemberROii.count():
            decemberROi=Sales.objects.filter(DateTime__year='2022',DateTime__month='12' or '11',CusName=decemberROii[deccountRO]).annotate(date=TruncDate('DateTime'),day=TruncDay('DateTime'),hour=TruncHour('DateTime'),minute=TruncMinute('DateTime')).values_list('date', flat=True).distinct().count()
            if decemberROi==2:
                MAINdecROcounts+=1
            deccountRO+=1
