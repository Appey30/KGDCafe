...
from .models import Brandcolor, ButtonColor, messengerbag, timesheet, Acceptorder, Rejectorder,Customer, acknowledgedstockorder,submitstockorder, user1, Categories, Sizes, Subcategories, PSizes, punchedprod,punchedprodso,queue1, queue2, queue3, Sales, Dailysales, couponlist
...
def selectplatform(fbid, received_postback):
    ...
    messageattachment = {
        ...
        "payload": {
            ...
            "text": "Welcome to KGD E-Cafe Ma'am/Sir "+userdetailsfirstname+", where do you prefer to order? \n Tap a button to answer",
            "buttons": [
                {
                "type":"web_url",
...
def selectorder(fbid, received_postback):
    ...
    if len(elementsprom) > 0:
        ...
        statuscategmt = requests.post(post_message_url, headers={"Content-Type": "application/json"},data=response_msgcategprom)
    ...
    messageattachmentprom = {
        "attachment": {
          ...
          "payload": {
            "template_type": "generic",
            "elements": elementsprom
          }
        }
    }
    response_msgprom = json.dumps({
    "recipient":{"id":fbid}, 
    "message":messageattachmentprom
    })

    statusprom = requests.post(post_message_url, headers={"Content-Type": "application/json"},data=response_msgprom)
    print(statusprom.json())
    ######  PROMO  #######
...
def messengercafe(request, product_id):
    ...
    print('Category: ',itembuttons.Category)
...
def customize(request):
    ...
    if is_ajax(request=request) and request.POST.get('button_colorcustom'):
        ...
        button_color_objadd = ButtonColor.objects.create(user=request.user.id, color = button_coloradd, cardcolor = card_coloradd, textcolor = text_coloradd, backgroundcolor = background_coloradd)
...
def Onlineordersystem(request, admin_id):
        ...
        if messengerredirect:
            user_details_url = "https://graph.facebook.com/v15.0/me/?fields=first_name,last_name,id&access_token=%s"%os.environ.get('USER_ACCESS_TOKEN')
        ...
        settings.SOCIAL_AUTH_FACEBOOK_SECRET = os.environ.get('SOCIAL_AUTH_FACEBOOK_SECRET')  # App Secret
        promocodegeti=request.GET.get('prmcd', '')
        #if messenger or messengerredirect:
        #    settings.SITE_URL = 'http://www.mydomain.com'
        if promocodegeti:
...
def staffthree(request):
    ...
    if request.method == 'POST':
        ...
        if image_data:
            image_data = image_data.replace('data:image/png;base64,', '') # remove the data URI prefix
            image = Image.open(io.BytesIO(base64.b64decode(image_data)))
            # TODO: Perform face recognition on the image and retrieve the employee ID
            # For testing purposes, we'll just return a random employee ID
            employee_id = 'ABC123'
